  ...
  <div class="container-fluid">
    <div class="page-header mb-0 mt-0">
      <div class="pull-left">
        <h4>广告内容管理</h4>
        <breadcrumb
                v-bind:items="[{text: '内容管理'},{text:'广告内容管理'},{text: '编辑广告内容', active: true}]"/>
      </div>
    ...
    <div class="panel panel-white">
      <div class="panel-body">
        <h4>编辑广告内容</h4>
        <hr>
        <form class="form-horizontal mt-2 update-link-form" method="post" novalidate="novalidate">
          <input type="hidden" name="id" v-model="link.id">
          <div class="form-group">
            <label for="title" class="control-label col-lg-2">标题</label>
            <div class="col-lg-10">
              <input class="form-control" id="title" name="title" type="text" required="" aria-required="true" v-model="link.title">
            </div>
          </div>
          <div class="form-group">
            <label for="position" class="control-label col-lg-2">位置</label>
            <div class="col-lg-10">
              <select class="form-control" id="position" name="position" v-model="link.position">
          ...
          <div class="form-group">
            ...
            <div class="col-lg-10">
              <input class="form-control" id="url" name="url" type="text" required="" aria-required="true" v-model="link.url">
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-lg-10">
              <label class="radio-inline">
                <input type="radio" name="targetNew" v-bind:value="true" v-model="link.targetNew"> 是
              </label>
              <label class="radio-inline">
                <input type="radio" name="targetNew" v-bind:value="false" v-model="link.targetNew"> 否
              </label>
            </div>
          </div>
          <div class="form-group ">
            ...
            <div class="col-md-8">
              <div class="list-img list-img-multiple clearfix" id="upload">
                <input class="upload_input" name="image" type="hidden" v-model="link.image">
  ...
  export default{
    ...
    methods: {
      getLink: function () {
        let self = this;
        self.ajax({
          url: api.content.link.get.url(self.$route.params.id),
      ...
      submitForm: function () {
        let self = this;
        $.each($('.update-link-form'), function (i, obj) {
          $(obj).validate({
            rules: {
              title: {
                required: true
            ...
            submitHandler: function (form) {
              self.ajax({
                type: 'POST',
                url: api.content.link.update.url,
