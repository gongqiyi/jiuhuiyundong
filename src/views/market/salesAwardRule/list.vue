  ...
  <div class="container-fluid">
    <div class="page-header mb-0 mt-0">
      <div class="pull-left">
        <h4>客户经理邀请奖励管理</h4>
        <breadcrumb
                v-bind:items="[{text: '推广管理'},{text:'客户经理邀请奖励管理',active:true}]"/>
      </div>
    </div>
    <div class="panel panel-white">
      <div class="panel-body">
        <form class="form-inline dk-search-list-bar rule-form">
          ...
          <div class="form-group">
            <router-link to="/market/salesAwardRule/create" class="btn btn-default btn-info">添加</router-link>
          </div>
        </form>
        <div class="table-overflow">
          <table id="js-datatable_example" class="table table-bordered table-middle table-hover mt-1-5">
            ...
            <tr v-for="item in pageList.list">
              ...
              <td align="center">
                <router-link class="btn btn-sm btn-primary" v-bind:to="'/market/salesAwardRule/update/' + item.id">编辑
                </router-link>
              </td>
            </tr>
            </tbody>
  ...
  export default{
    ...
    methods: {
      ...
      getRuleList: function (data) {
        let self = this;
        self.ajax({
          url: api.market.salesAwardRule.list.url,
          data: data,
          success: function (res) {
            self.pageList = res.salesRulePageList;
            self.currentIndex = res.salesRulePageList.pageIndex;
          }
        });
      },
      // 组合描述
      combineDescription: function (ruleType, ruleModel) {
        ...
        if (ruleType == 1) {
          ...
          desc = '被邀请人每比线上全额投保获得' +
                  ...
                  '元，每比分期投保获得' +
                  (ruleModel.perInstallmentOffline || 0) + '元';
        }
        else if (ruleType == 3) {
