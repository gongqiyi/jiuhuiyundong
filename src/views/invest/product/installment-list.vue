  ...
  <div class="container-fluid">
    <div class="page-header mb-0 mt-0">
      <div class="pull-left">
        <h4>分期投保产品管理</h4>
        <breadcrumb
                v-bind:items="[{text: '投保管理'},{text:'产品管理'},{text:'分期投保产品管理',active:true}]"/>
      </div>
    </div>
    <div class="panel panel-white">
      <div class="panel-body">
        <form class="form-inline dk-search-list-bar product-installment-form">
          ...
          <div class="form-group">
            <router-link to="/invest/product/installment/create" class="btn btn-default btn-info">添加</router-link>
          </div>
        </form>
        <table id="js-datatable_example" class="table table-bordered table-middle table-hover mt-1-5">
          ...
          <tr v-for="item in pageList.list">
            ...
            <td align="center">
              ...
              <router-link class="btn btn-sm btn-primary" v-bind:to="'/invest/product/installment/update/' + item.id">编辑
              </router-link>
            </td>
          </tr>
          </tbody>
  ...
  export default{
    ...
    methods: {
      ...
      getProductList: function (data) {
        let self = this;
        self.ajax({
          url: api.invest.product.installment.list.url,
          data: data,
          success: function (res) {
            self.pageList = res.investProductInstallmentPageList;
            self.currentIndex = res.investProductInstallmentPageList.pageIndex;
          }
        });
      },
      // 搜索
      search: function (data) {
        ...
        let formData = commonApp.tools.getFromData($('.product-installment-form'));
      ...
      open: function (id) {
        ...
        layer.confirm("确定要上架该产品吗？", function () {
          self.ajax({
            type: 'POST',
            url: api.invest.product.installment.open.url(id),
            success: function (res) {
      ...
      close: function (id) {
        ...
        layer.confirm("确定要下架该产品吗？", function () {
          self.ajax({
            type: 'POST',
            url: api.invest.product.installment.close.url(id),
            success: function (res) {
              layer.msg(res.message, {time: 1000}, function () {
                if (res.code === 200) {
                  self.search();
